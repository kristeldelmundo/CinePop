  ...
  return (
    <div className={clsx(
      ...
      <div className="w-14 h-20 rounded-xl overflow-hidden flex-shrink-0 bg-rose-50">
        {item.poster ? (
          <Image src={item.poster} alt={item.title} width={56} height={80} className="w-full h-full object-cover" />
        ) : (
      ...
      <div className="flex-1 min-w-0 py-0.5">
        <div className="flex items-start justify-between gap-2 mb-1">
          <h3 className="font-medium text-gray-800 text-sm leading-tight truncate">{item.title}</h3>
          <span className={clsx('text-xs px-2 py-0.5 rounded-full flex-shrink-0', item.type === 'movie' ? 'pill-movie' : 'pill-tv')}>
        ...
        <div className="flex items-center gap-2 mb-1.5">
          ...
          {item.genre && <span className="text-xs text-gray-400 truncate">{item.genre.split(',')[0]}</span>}
          {item.rating && item.rating !== 'N/A' && (
            <span className="text-xs text-amber-500 font-medium">★ {item.rating}</span>
          )}
        </div>

        {item.plot && (
          <p className="text-xs text-gray-400 leading-relaxed line-clamp-2">{item.plot}</p>
        )}
        ...
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-center gap-1">
            <span className={clsx(
              ...
              item.added_by === 'K' ? 'bg-rose-100 text-rose-500' : 'bg-purple-100 text-purple-500'
            )}>
              {item.added_by}
            </span>
