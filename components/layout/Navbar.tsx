...
export default function Navbar() {
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) {
      ...
      if (circleRef.current && !circleRef.current.contains(e.target as Node)) setCircleMenuOpen(false);
    }
    ...
    <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-rose-100">
      <div className="w-full px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-1.5 flex-shrink-0">
          ...
          <span className="font-display text-lg font-medium text-rose-600 italic hidden sm:inline">CinePop</span>
        </Link>
        ...
        <div className="flex items-center gap-1 flex-1 justify-center min-w-0 px-1">
          ...
          {navItems.map(({ href, label, icon: Icon, tourId }) => (
            <Link
              ...
              id={tourId}
              className={clsx(
                "flex items-center justify-center w-9 h-9 sm:w-auto sm:h-auto sm:px-3 sm:py-1.5 sm:gap-1.5 rounded-full text-sm font-medium transition-all flex-shrink-0",
                pathname === href
        ...
        {user ? (
          <div className="flex items-center gap-0.5 flex-shrink-0">
            ...
            <div className="relative" ref={menuRef} id="tour-nav-profile-chip">
              <button
                ...
                className="flex items-center gap-0.5 p-1 rounded-full hover:bg-rose-50 transition-colors"
              >
                {profile?.avatar_url ? (
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-rose-100 shadow-xl shadow-rose-100/50 overflow-hidden z-50">
          ...
          <Link href="/login" className="text-sm font-medium text-rose-500 hover:text-rose-600 flex-shrink-0 px-3 py-1.5 rounded-full bg-rose-50 hover:bg-rose-100 transition-colors">
            Log in
          </Link>
        )}
      </div>
